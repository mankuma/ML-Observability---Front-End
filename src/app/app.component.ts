...
export class AppComponent implements OnInit {
  ...
  checkAndSetActiveAccount() {
    ...
    if (!activeAccount && this.authService.instance.getAllAccounts().length > 0) {
      ...
      let authId = 'msal.token.keys' + '.' + user?.idTokenClaims?.aud;
      let fecthToken = localStorage.getItem(authId);
      let saveToken: any = fecthToken?.split(',');
      let token: any = JSON.parse(saveToken).accessToken[0];
      let bearerToken: any = localStorage.getItem(token)?.split(',');
      localStorage.setItem('Authorization', JSON.parse(bearerToken).secret);
    }
  }
